...
): Promise<Nullable<TodosResponseType>> => {
  try {
    const snapshot = await get(child(ref(database), `/${userUid}/${TODOS_PATH}`));
    if (snapshot.exists()) {
...
export const addTodo = createAsyncThunk(
  `${TODOS_PATH}/addTodo`,
  async (payload: AddTodoPayloadType,
): Promise<ITodo> => {
  try {
    const todo = {
    ...
    await set(ref(database, `/${payload.userUid}/${TODOS_PATH}/${todo.id}`), todo);
...
export const updateTodo = createAsyncThunk(
  ...
  async (payload: UpdateTodoPayloadType,
): Promise<ITodo> => {
  try {
    const todo = {
      id: payload.id,
    ...
    await update(ref(database, `/${payload.userUid}/${TODOS_PATH}/${todo.id}`), todo);
...
export const removeTodo = createAsyncThunk(
  ...
  try {
    await remove(ref(database, `/${payload.userUid}/${TODOS_PATH}/${payload.todoId}`));
